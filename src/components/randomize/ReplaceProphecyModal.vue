...
export default defineComponent({
  ...
  setup() {
    ...
    const specifying = computed(() => randomizerStore.specifyingReplacementProphecy);
    ...
    const filteredCards = computed(() => {
      ...
      let matchedCards = Cards.getAllProphecies(allCards);
      matchedCards = matchedCards.filter(Cards.filterByIncludedSetIds(setIdsToUse));
      
      const currentProphecyId = randomizerStore.kingdom.prophecy ? randomizerStore.kingdom.prophecy.id : null;
      matchedCards = matchedCards.filter(card => {
        const inKingdom = currentProphecyId === card.id;
    ...
    const hoveredCard = ref<Prophecy | null>(null);
    const mouseX = ref(0);
    const mouseY = ref(0);

    const handleMouseEnterCard = (card: Prophecy, event: MouseEvent) => {
    ...
    const handleEscapeKey = () => {
      randomizerStore.CLEAR_SPECIFYING_REPLACEMENT_PROPHECY();
    };
    const handleCancel = () => {
      randomizerStore.CLEAR_SPECIFYING_REPLACEMENT_PROPHECY();
    };

    const handleSelectCard = (card: Prophecy) => {
      randomizerStore.REPLACE_SPECIFYING_PROPHECY(card);
      hoveredCard.value = null;
    };
