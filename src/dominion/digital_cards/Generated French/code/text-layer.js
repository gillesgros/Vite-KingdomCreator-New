...
var TextLayer = (function (_StackViewLayer) {
  ...
  function TextLayer(rootNode, cardStack) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (TextLayer.__proto__ || Object.getPrototypeOf(TextLayer)).call(
        this,
        rootNode,
        cardStack
      )
  ...
  _createClass(TextLayer, [
    {
      ...
      value: function redraw(w, h) {
        ...
        var isInherited =
          card.cardName === CardNames.ESTATE &&
          card.types.includes(Types.ACTION);
        var shouldHide =
          cardSize !== CardSizes.FULL ||
          cardName.isBack() ||
          this.cardStack.hasFilter("appears-anonymous") ||
          this.cardStack.hasFilter("inverted-card") ||
          !(cardName in LANGUAGE.getCardText);
        ...
        var measureBlock = function measureBlock(
          ...
          switch (block.type) {
            ...
            case "shield":
              ctx.font = "bold " + measurementSize + "px " + fontFamily;
              return (
                ctx.measureText(block.inner).width +
                measurementSize * victoryRatio
              );
            case "bigshield":
              ctx.font =
                "bold " + measurementSize * bigLineFactor + "px " + fontFamily;
              return (
                ctx.measureText(block.inner).width +
                measurementSize * victoryRatio
              );
        ...
        var measureLine = function measureLine(
          line,
          fontFamily,
          measurementSize
        ) {
          ...
          var lineText =
            lineIsBold || lineIsItalics ? line.slice(1, line.length - 1) : line;
          ...
          if (!lineIsSeparator && !lineIsBlank) {
            ...
            blocks.forEach(function (block) {
              ...
              if (block === "shield" || block === "bigshield")
                totalLength += measureBlock(
                  block,
                  "Minion-Bold",
                  measurementSize
                );
              else
                totalLength += measureBlock(block, fontFamily, measurementSize);
            });
          }
        ...
        var renderBlock = function renderBlock(fontFamily, boldFontFamily) {
          return function (block) {
            ...
            var handleTerminalPunctuation =
              function handleTerminalPunctuation() {
                var punctuation = "+-,;.";
                if (punctuation.includes(node.innerHTML[0]))
                  node.classList.add("no-left-margin");
                if (
                  punctuation.includes(
                    node.innerHTML[node.innerHTML.length - 1]
                  )
                )
            ...
            switch (block.type) {
              ...
              case "bigcoin":
                ...
                if (block.inner.match(/\d+D/))
                  cost.debt = parseInt(block.inner.match(/(\d+)D/)[1]);
                else if (block.inner === "D") forceShow = "debt";
                else if (block.inner === "") forceShow = "coin";
                else cost.coin = parseInt(block.inner);
        ...
        var renderLine = function renderLine(fontFamily, boldFontFamily) {
          return function (line) {
            ...
            var renderedBlocks = blocks.map(
              renderBlock(fontFamily, boldFontFamily)
            );
        ...
        var cardTextRaw = isInherited
          ? LANGUAGE.getPhrase[Phrases.INHERITANCE_INSTRUCTIONS]
          : LANGUAGE.getCardText[cardName];
        var lines = cardTextRaw.split("//");

        var bodyFont = LANGUAGE.getEnvironment.hasOwnProperty("bodyFont")
          ? LANGUAGE.getEnvironment.bodyFont
          : "Times New Roman";
        var boldFont = LANGUAGE.getEnvironment.hasOwnProperty("boldFont")
          ? LANGUAGE.getEnvironment.boldFont
          : "Times New Roman";
        ...
        var capSize =
          bbox.height *
          (cardName.isLandscape() ? maxLandscapeFontFactor : maxFontFactor);
        var measuredSize = Math.min(
          (measurementSize * bbox.height) / totalHeight,
